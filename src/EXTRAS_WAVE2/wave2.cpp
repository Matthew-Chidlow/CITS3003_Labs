...
init()
{
    ...
    GLuint program = InitShader( "vwave.glsl", "fwave.glsl" );
    ...
    MVP_loc = glGetUniformLocation(program, "ModelViewProjectionMatrix");
...
static void draw(void)
{
    ...
    mat4 MVP, MV;
    MV=LookAt(vec4(2.0, 2.0, 2.0, 1.0), vec4(0.5, 0.0, 0.5, 1.0), vec4(0.0, 1.0, 0.0, 1.0));
    MVP=Ortho(-0.75,0.75,-0.75,0.75,-5.5,5.5)*MV;
    glUniformMatrix4fv(MVP_loc, 1, GL_TRUE, MVP);
    glUniformMatrix4fv(MV_loc, 1, GL_TRUE, MVP);
...
int main(int argc, char** argv)
{
    ...
    init();
    InitShader("vwave.glsl", "fwave.glsl");
