...
//
// When running this program, click the middle mouse button inside the
// window to bring up the menu. This is an interesting program which contains
// some advanced features.  The last menu item is for quitting the program.
//
// Du Huynh
...
void reshape( int width, int height )
{
    ...
    point4 eye = vec4( 2.0, 1.0, 6.0, 1.0 ); // position of the eye (camera)
    point4 at = vec4( 0.0, 0.0, 0.0, 1.0 );  // position of the point to look at
    vec4 up = vec4( 0.0, 1.0, 0.0, 0.0 );    // the up-direction of the eye

    projection = Ortho( -2.5, 2.5, -2.5, 2.5, 2.0, 16.0 );
    model_view = LookAt( eye, at, up );

    // for debugging
    fprintf(stderr, "model_view = \n");
    printm(model_view);  // inline function in mat.h
